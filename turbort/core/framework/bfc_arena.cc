...
#include "turbort/core/framework/bfc_arena.h"
#include <map>
#include <type_traits>
...
turbo::Status BFCArena::Extend(size_t rounded_bytes) {
  ...
  if (rounded_bytes > available_bytes) {
    return turbo::Status(kTBRT, turbo::kResourceExhausted, turbo::StrCat("Available memory of ", available_bytes,
  ...
  if (mem_addr == nullptr) {
    return turbo::Status(
        kTBRT, turbo::kResourceExhausted, turbo::StrCat("Failed to allocate memory for requested buffer of size ",
        rounded_bytes));
  }
...
void *BFCArena::AllocateRawInternal(size_t num_bytes,
  ...
  if (status.ok()) {
    ...
    if (ptr != nullptr) {
      ...
      status = turbo::Status(kTBRT, turbo::kResourceExhausted,
                               turbo::StrCat("Failed to find a free memory block despite "
