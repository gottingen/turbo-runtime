...
static constexpr unsigned short int kTBRT = 1;
...
// __PRETTY_FUNCTION__
#define BRT_WHERE ::turbort::CodeLocation(__FILE__, __LINE__, __FUNCTION__)

#define BRT_WHERE_WITH_STACK \
  ::turbort::CodeLocation(__FILE__, __LINE__, __PRETTY_FUNCTION__, ::turbort::GetStackTrace())
...
#define BRT_NOT_IMPLEMENTED(...)                                                \
  do {                                                                          \
    ::turbort::PrintFinalMessage(                                                   \
        ::turbort::NotImplementedException(::turbort::MakeString(__VA_ARGS__)).what()); \
    abort();                                                                    \
  } while (false)
...
#define BRT_THROW_EX(ex, ...)                                                      \
  do {                                                                             \
    ::turbort::PrintFinalMessage(                                                      \
        ::turbort::MakeString(#ex, "(", ::turbort::MakeString(__VA_ARGS__), ")").c_str()); \
    abort();                                                                       \
  } while (false)
...
// Just in order to mark things as not implemented. Do not use in final code.
#define BRT_NOT_IMPLEMENTED(...) \
  throw ::turbort::NotImplementedException(::turbort::MakeString(__VA_ARGS__))
...
#define BRT_MAKE_STATUS(category, code, ...) \
  ::turbo::Status(                     \
      ::turbort::common::category, ::turbort::common::code, ::turbort::MakeString(__VA_ARGS__))

// Check condition. if met, return status.
#define BRT_RETURN_IF(condition, ...)                                                        \
  if (condition) {                                                                           \
    return ::turbo::Status(::turbort::kTBRT,                                         \
                                 ::turbo::k,                                        \
                                 ::turbort::MakeString(BRT_WHERE.ToString(), " ", __VA_ARGS__)); \
  }
...
#define BRT_RETURN_IF_ERROR_SESSIONID(expr, session_id)                              \
  do {                                                                               \
    auto _status = (expr);                                                           \
    if ((!_status.IsOK())) {                                                         \
      ::turbort::LogRuntimeError(session_id, _status, __FILE__, __FUNCTION__, __LINE__); \
      return _status;                                                                \
    }                                                                                \
  } while (0)
...
#define BRT_THROW_IF_ERROR(expr)                                            \
  do {                                                                      \
    auto _status = (expr);                                                  \
    if ((!_status.IsOK())) {                                                \
      ::turbort::LogRuntimeError(0, _status, __FILE__, __FUNCTION__, __LINE__); \
      BRT_THROW(_status);                                                   \
    }                                                                       \
  } while (0)
