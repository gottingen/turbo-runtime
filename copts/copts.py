"""Turbort compiler options.

This is the source of truth for Turbort compiler options.  To modify Turbort
compilation options:

  (1) Edit the appropriate list in this file based on the platform the flag is
      needed on.
  (2) Run `<path_to_turbort>/copts/generate_copts.py`.

The generated copts are consumed by configure_copts.bzl and
TurbortConfigureCopts.cmake.
"""
...
TURBORT_GCC_TEST_ADDITIONAL_FLAGS = [
...
TURBORT_LLVM_TEST_ADDITIONAL_FLAGS = [
    "-Wno-deprecated-declarations",
...
COPT_VARS = {
    "TURBORT_GCC_FLAGS": TURBORT_GCC_FLAGS,
    "TURBORT_GCC_TEST_FLAGS": GccStyleFilterAndCombine(
        TURBORT_GCC_FLAGS, TURBORT_GCC_TEST_ADDITIONAL_FLAGS),
    "TURBORT_LLVM_FLAGS": TURBORT_LLVM_FLAGS,
    "TURBORT_LLVM_TEST_FLAGS": GccStyleFilterAndCombine(
        TURBORT_LLVM_FLAGS, TURBORT_LLVM_TEST_ADDITIONAL_FLAGS),
    "TURBORT_CLANG_CL_FLAGS":
        MSVC_BIG_WARNING_FLAGS + MSVC_DEFINES,
    "TURBORT_CLANG_CL_TEST_FLAGS":
        MSVC_BIG_WARNING_FLAGS + MSVC_DEFINES + TURBORT_LLVM_TEST_ADDITIONAL_FLAGS,
    "TURBORT_MSVC_FLAGS":
    ...
    "TURBORT_RANDOM_HWAES_ARM64_FLAGS": ["-march=armv8-a+crypto"],
    "TURBORT_RANDOM_HWAES_ARM32_FLAGS": ["-mfpu=neon"],
